    ...
    def forward(self, pixel_values, input_boxes):
        ...
        with torch.no_grad():
            for i, encoder in enumerate(self.image_encoders): 
                image_embeddings.append(encoder(pixel_values[i])) # (B, 256, 64, 64)
...
class SumSamModel(pl.LightningModule):
    def __init__(self, model_name, learning_rate=1e-3, startOver=True, normalize=True, models = [], HQ = False):
        super(SumSamModel, self).__init__()

        image_encoders = []
        for i in range(len(models)):
            image_encoders.append(models[i].model.image_encoder)
        

        if not HQ:
            self.model = SumSAM(image_encoders, models[0].model.mask_decoder, models[0].model.prompt_encoder, normalize=normalize, coefficients=[0.5,0.5]) 
    
            # Freeze specific weights
            for name, param in self.model.named_parameters():
                #if name.startswith("image_encoder") or name.startswith("prompt_encoder"):
                if "sfg" in name or "decoder" in name:
...
class SAMDataset(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    imageVV = np.array(item["imageVV"])
    imageVH = np.array(item["imageVH"])
    ...
    inputsVV = self.processor(imageVV, input_boxes=input_boxes,do_normalize = self.normalize, return_tensors="pt", do_rescale = self.rescale)
    inputsVH = self.processor(imageVH, input_boxes=input_boxes,do_normalize = self.normalize, return_tensors="pt", do_rescale = self.rescale)

    # remove batch dimension which the processor adds by default
    inputsVV = {k:v for k,v in inputsVV.items()}
    inputsVH = {k:v for k,v in inputsVH.items()}

    pixel_values = [inputsVV["pixel_values"], inputsVH["pixel_values"]]
    input_boxes = [inputsVV["input_boxes"], inputsVH["input_boxes"]]

    inputs = inputsVV.copy()
    inputs["pixel_values"] = pixel_values
    inputs["input_boxes"] = input_boxes

    ground_truth_mask = torch.from_numpy(ground_truth_mask)
    ground_truth_mask = [ground_truth_mask, ground_truth_mask]
    ...
    if self.test:
        ...
        for mask in inputs["ground_truth_mask"].numpy():
            ...
            if len(boxes) == 0:
                ...
                min_size = 50
                max_size = 250
                # Ensure the random box is not too large.
