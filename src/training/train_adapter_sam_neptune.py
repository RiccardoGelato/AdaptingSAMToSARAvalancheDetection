...
from model.minor_models.selfSupervisedTwoEncodersModel import selfSupSamModel, SAMDataset
...
train_dataset_sam = SAMDataset3(dataset=train_dataset, processor=processor, augment=True, type = InputTypes.VV)
...
# Create an instance of the SAMDataset
val_dataset_sam = SAMDataset3(dataset=val_dataset, processor=processor, augment=False, type = InputTypes.VV)
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename='sam-adapters-4-{epoch:02d}-{val_loss:.3f}-{val_iou:.3f}',  # Filename template
...
checkpoint_callback_2 = ModelCheckpoint(
    ...
    dirpath=CHECKPOINT_DIR,  # Directory to save checkpoints
    filename='sam-adapters-4-loss-{epoch:02d}-{val_loss:.3f}-{val_iou:.3f}',  # Filename template
    save_top_k=2,  # Save the top 3 models
    mode='min'  # Mode can be 'min' or 'max'. Use 'min' for loss and 'max' for accuracy
)
...
#
#
#train_dataset_selfsup = load_from_disk('/home/gelato/Avalanche-Segmentation-with-Sam/code/dataprocessing/datasetTrainSelfSupFinal')
#
## Create an instance of the SAMDataset
#train_dataset_sam_selfsup = SAMDataset(dataset=train_dataset_selfsup, processor=processor, augment=True, type=InputTypes.Normal, target_type=InputTypes.Normal)
#
## Create a DataLoader instance for the training dataset
#train_dataloader_selfsup = DataLoader(train_dataset_sam_selfsup, batch_size=5, shuffle=True, drop_last=True, num_workers=8, collate_fn=custom_collate)
#
#standardSAM = LitSamModel(model_name="vit_b", normalize=True, learning_rate=1e-5, adapt_patch_embed=False, input_type=InputTypes.Normal, adapt=False)
#adapterSAM = LitSamModel(model_name="vit_b", normalize=True, learning_rate=1e-5, adapt_patch_embed=False, input_type=InputTypes.Normal, adapt=True)
#
#modelSelfSup = selfSupSamModel(adapterSAM.model.image_encoder, standardSAM.model.image_encoder, learning_rate=1e-5, normalize=True, input_type=InputTypes.Normal, target_input_type=InputTypes.Normal)
#
#trainer_selfsup = pl.Trainer(
#    max_epochs=10,  # Maximum number of epochs
#    accelerator='gpu',  # Use GPU if available
#    devices=1,  # Use GPUs if available
#)
#
#trainer_selfsup.fit(modelSelfSup, train_dataloader_selfsup)
#
#############################
#############################
#############################
...
model = LitSamModel(model_name="vit_b", normalize=True, learning_rate=1e-5, adapt_patch_embed=False, input_type=InputTypes.VV, adapt=True)
...
trainer = pl.Trainer(
    ...
    accelerator='gpu',  # Use GPU if available
    devices=1,  # Use GPUs if available
    callbacks=[checkpoint_callback, early_stopping_callback, lr_monitor, checkpoint_callback_2],  # Add the callbacks
