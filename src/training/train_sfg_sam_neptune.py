...
from model.sumModel import SumSamModel, SAMDataset3
...
PATH_VV = config['paths']['checkpoint_vv']
PATH_VH = config['paths']['checkpoint_vh']
...
train_dataset_sam = SAMDataset3(dataset=train_dataset, processor=processor, augment=True)
...
# Create an instance of the SAMDataset
val_dataset_sam = SAMDataset3(dataset=val_dataset, processor=processor, augment=False)
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename='sam-sfg-2-decoder-{epoch:02d}-{val_loss:.3f}-{val_iou:.3f}',  # Filename template
...
checkpoint_callback_2 = ModelCheckpoint(
    ...
    dirpath=CHECKPOINT_DIR,  # Directory to save checkpoints
    filename='sam-sfg-2-decoder-loss-{epoch:02d}-{val_loss:.3f}-{val_iou:.3f}',  # Filename template
    save_top_k=2,  # Save the top 3 models
...
#target_image_encoder = LitSamModel(model_name="vit_b", normalize=True, learning_rate=1e-5, adapt_patch_embed=False, input_type=InputTypes.VV, adapt=True).model.image_encoder
#
## Create an instance of the model architecture with the loaded configuration
...
sumModel = SumSamModel(models = [model1, model2], model_name="vit-b", normalize = True, HQ = False, learning_rate=1e-5)
...
trainer = pl.Trainer(
    ...
    callbacks=[checkpoint_callback_2, checkpoint_callback, early_stopping_callback, lr_monitor],  # Add the callbacks
    logger=neptune_logger  # Enable logging
)


# Train the model
trainer.fit(sumModel, train_dataloader, val_dataloader)
